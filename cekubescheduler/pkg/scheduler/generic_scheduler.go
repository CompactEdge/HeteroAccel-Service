...
func (g *genericScheduler) Schedule(ctx context.Context, extenders []framework.Extender, fwk framework.Framework, state *framework.CycleState, pod *v1.Pod) (result ScheduleResult, err error) {
	...
	// if err != nil && err.Error() == "ScoreMetric" {
	// 	goto SCHEDULE
	// } else if err != nil {
	// 	return result, err
	// }
...
func prioritizeNodes(
	...
	//	klog.InfoS("prioritizeNodes TEST")
	...
	if klog.V(10).Enabled() {
		...
		for plugin, nodeScoreList := range scoresMap {
			for _, nodeScore := range nodeScoreList {
				...
				if plugin == "NodeResourcesLeastAllocated" {
					if nodeScore.Score > 20 {
						klog.InfoS("TRUE", nodeScore.Name, nodeScore.Score)
					} else {
						klog.InfoS("FALSE", nodeScore.Name, nodeScore.Score)
					}
				}
			}
		}
