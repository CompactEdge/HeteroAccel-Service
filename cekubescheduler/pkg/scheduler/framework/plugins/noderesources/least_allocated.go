...
func (la *LeastAllocated) Score(ctx context.Context, state *framework.CycleState, pod *v1.Pod, nodeName string) (int64, *framework.Status) {
	...
	// nodeMetrics, err := la.handle.Metrics().MetricsV1beta1().NodeMetricses().List(ctx, metav1.ListOptions{})
	// if err != nil {
	...
	for i := range la.handle.GetNodeMetrics().Items {
		if nodeName == la.handle.GetNodeMetrics().Items[i].ObjectMeta.Name {
			nodeInfo.Requested.MilliCPU = la.handle.GetNodeMetrics().Items[i].Usage.Cpu().MilliValue()

		}
	}
	//elapsed := time.Since(start)
	//klog.InfoS("Score Calc: ", elapsed)
	...
	return func(requested, allocable resourceToValueMap) int64 {
		...
		for resource := range requested {
			...
			if resource == v1.ResourceCPU {
				...
				//klog.InfoS("Resource CPU: ", resource, requested[resource], allocable[resource])

			} else {
				resourceScore = leastRequestedScore(requested[resource], 0)
				//klog.InfoS("Resource OTHER: ", resource, requested[resource], allocable[resource])
